                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        ...
                                        $itemsql = "SELECT * FROM `cake` WHERE cakeId = $cakeId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $cakeName = $itemrow['cakeName'];
                                        $cakePrice = $itemrow['cakePrice'];
                                        $cakeDesc = $itemrow['cakeDesc'];
                                        $cakeCategorieId = $itemrow['cakeCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="img/cake-'.$cakeId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> <a href="#" class="text-dark d-inline-block align-middle">'.$cakeName. '</a></h5><span class="text-muted font-weight-normal font-italic d-block"> ' .$cakePrice. 'đ-</span>
                                                    </div>
                                                    </div>
                                                </th>
