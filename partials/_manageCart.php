...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `viewcart` WHERE cakeId = '$itemId' AND `userId`='$userId'";
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `viewcart` (`cakeId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `viewcart` WHERE `cakeId`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    $cakeId = $addrow['cakeId'];
                    $itemQuantity = $addrow['itemQuantity'];
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `cakeId`, `itemQuantity`) VALUES ('$orderId', '$cakeId', '$itemQuantity')";
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        $cakeId = $_POST['cakeId'];
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `viewcart` SET `itemQuantity`='$qty' WHERE `cakeId`='$cakeId' AND `userId`='$userId'";
