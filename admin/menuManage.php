						...
						<table class="table table-bordered table-hover mb-0">
                            ...
                            <?php
                                ...
                                while($row = mysqli_fetch_assoc($result)){
                                    ...
                                    echo '<tr>
                                            <td class="text-center">' .$cakeCategorieId. '</td>
                                            <td>
                                                <img src="/Cake/img/cake-'.$cakeId. '.jpg" alt="image for this item" width="150px" height="150px">
                                            </td>
                                            <td>
                                                <p>Name : <b>' .$cakeName. '</b></p>
                                                <p>Description : <b class="truncate">' .$cakeDesc. '</b></p>
                                                <p>Price : <b>' .$cakePrice. 'đ	</b></p>
                                            </td>
                                            <td class="text-center">
												<div class="row mx-auto" style="width:112px">
													<button class="btn btn-sm btn-primary" type="button" data-toggle="modal" data-target="#updateItem' .$cakeId. '">Edit</button>
...
<?php 
    $cakesql = "SELECT * FROM `cake`";
    $cakeResult = mysqli_query($conn, $cakesql);
    while($cakeRow = mysqli_fetch_assoc($cakeResult)){
        $cakeId = $cakeRow['cakeId'];
        $cakeName = $cakeRow['cakeName'];
        $cakePrice = $cakeRow['cakePrice'];
        $cakeCategorieId = $cakeRow['cakeCategorieId'];
        $cakeDesc = $cakeRow['cakeDesc'];
?>

<!-- Modal -->
<div class="modal fade" id="updateItem<?php echo $cakeId; ?>" tabindex="-1" role="dialog" aria-labelledby="updateItem<?php echo $cakeId; ?>" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color: rgb(111 202 203);">
        <h5 class="modal-title" id="updateItem<?php echo $cakeId; ?>">Item Id: <?php echo $cakeId; ?></h5>
	  	...
	  	<form action="partials/_menuManage.php" method="post" enctype="multipart/form-data">
		    <div class="text-left my-2 row" style="border-bottom: 2px solid #dee2e6;">
		   		<div class="form-group col-md-8">
					...
					<small id="Info" class="form-text text-muted mx-3">Please .jpg file upload.</small>
					<input type="hidden" id="cakeId" name="cakeId" value="<?php echo $cakeId; ?>">
					<button type="submit" class="btn btn-success my-1" name="updateItemPhoto">Update Img</button>
				</div>
				<div class="form-group col-md-4">
					<img src="/Cake/img/cake-<?php echo $cakeId; ?>.jpg" id="itemPhoto" name="itemPhoto" alt="item image" width="100" height="100">
				</div>
			</div>
		</form>
		<form action="partials/_menuManage.php" method="post">
            <div class="text-left my-2">
                ...
                <input class="form-control" id="name" name="name" value="<?php echo $cakeName; ?>" type="text" required>
            </div>
			<div class="text-left my-2 row">
				<div class="form-group col-md-6">
                	<b><label for="price">Price</label></b>
                	<input class="form-control" id="price" name="price" value="<?php echo $cakePrice; ?>" type="number" min="1" required>
				</div>
				<div class="form-group col-md-6">
					<b><label for="catId">Category Id</label></b>
                	<input class="form-control" id="catId" name="catId" value="<?php echo $cakeCategorieId; ?>" type="number" min="1" required>
				</div>
            </div>
            <div class="text-left my-2">
                <b><label for="desc">Description</label></b>
                <textarea class="form-control" id="desc" name="desc" rows="2" required minlength="6"><?php echo $cakeDesc; ?></textarea>
            </div>
            <input type="hidden" id="cakeId" name="cakeId" value="<?php echo $cakeId; ?>">
